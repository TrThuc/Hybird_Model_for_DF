...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Maximum frequency on the Y axis for ALL datasets; omit for automatic scaling.",
    )
    # ymax tùy chỉnh riêng từng dataset
    parser.add_argument("--ymax-celebdf", type=float, default=None)
    parser.add_argument("--ymax-dfd", type=float, default=None)
    parser.add_argument("--ymax-dfdc", type=float, default=None)
    parser.add_argument("--ymax-dfdcp", type=float, default=None)
    parser.add_argument("--ymax-faceshifter", type=float, default=None)
    parser.add_argument("--ymax-uadfv", type=float, default=None)
...
def main() -> None:
    ...
    # Ưu tiên ymax riêng -> ymax chung -> None (tự động)
    ymax_map = {}
    for key, name in DATASETS:
        specific_ymax = getattr(args, f"ymax_{key}", None)
        chosen_ymax = specific_ymax if specific_ymax is not None else args.y_max
        ymax_map[key] = None if chosen_ymax is None or chosen_ymax < 0 else chosen_ymax
    ...
    print("All valid prediction rows were used; ymax limits applied per dataset.")
