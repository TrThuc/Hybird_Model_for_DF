...
def make_rel_path(path, root):
    """
    Convert absolute path to relative path from dataset root.
    Use forward slash so JSON works on Windows/Linux/Kaggle/Colab.
    """
...
def generate_dataset_file(dataset_name, dataset_root_path, output_file_path, compression_level='c23', perturbation = 'end_to_end'):
    ...
    if dataset_name == 'FaceForensics++' or dataset_name == 'DeepFakeDetection' or dataset_name == 'FaceShifter': 
            ...
            for compression_level in os.scandir(os.path.join(dataset_path, 'original_sequences', 'youtube')):
                ...
                for video_path in os.scandir(os.path.join(dataset_path, 'original_sequences', 'youtube', compression_level, 'frames')):
                    if video_path.is_dir():
                        ...
                        frame_paths = [
                            make_rel_path(os.path.join(video_path, frame.name), dataset_root_path)
                            for frame in os.scandir(video_path)
                            if frame.name.lower().endswith(('.png', '.jpg', '.jpeg'))
                        ]
            ...
            # Same operations for DeepfakeDetection real dataset
                ...
                for compression_level in os.scandir(actors_root):
                    if compression_level.is_dir() and compression_level.name in ["c23", "c40", "raw"]:
                        ...
                        for video_path in os.scandir(os.path.join(actors_root, compression_level, 'frames')):
                            if video_path.is_dir():
                                video_name = video_path.name
                                frame_paths = [
                                    make_rel_path(
                ...
                print("Skip missing folder:", actors_root)
        ...
        if os.path.isdir(os.path.join(dataset_path, 'manipulated_sequences')):
            valid_ffpp_labels = ['Deepfakes', 'Face2Face', 'FaceSwap', 'NeuralTextures', 'DeepFakeDetection', 'FaceShifter']
            ...
            for label_dir in os.scandir(os.path.join(dataset_path, 'manipulated_sequences')):
                ...
                if label not in valid_ffpp_labels:
                    print("Skip unused manipulated folder:", label)
                    continue
                ...
                for comp_entry in os.scandir(os.path.join(dataset_path, 'manipulated_sequences', label)):
                    if comp_entry.is_dir() and comp_entry.name in ["c23", "c40", "raw"]:
                        comp_name = comp_entry.name

                        dataset_dict['FaceForensics++'][ff_dict[label]]['train'][comp_name] = {}
                        dataset_dict['FaceForensics++'][ff_dict[label]]['test'][comp_name] = {}
                        dataset_dict['FaceForensics++'][ff_dict[label]]['val'][comp_name] = {}
                        ...
                        if not os.path.isdir(frames_root):
                            print("Skip missing frames folder:", frames_root)
                            continue
                        ...
                        for video_path in os.scandir(frames_root):
                            if video_path.is_dir():
                                ...
                                try:
                                    mode = video_to_mode[video_name]
                                    dataset_dict['FaceForensics++'][ff_dict[label]][mode][comp_name][video_name] = {
                                    'label': ff_dict[label],
                                    'frames': frame_paths,
                                    'masks': mask_frames_paths
                                }
                                except:
                                    dataset_dict['FaceForensics++'][ff_dict[label]]['train'][comp_name][video_name] = {
                                        'label': ff_dict[label],
                                        'frames': frame_paths,
                                        'masks': mask_frames_paths
                                    }
                                    dataset_dict['FaceForensics++'][ff_dict[label]]['val'][comp_name][video_name] = {
                                        'label': ff_dict[label],
                                        'frames': frame_paths,
                                        'masks': mask_frames_paths
                                    }
                                    dataset_dict['FaceForensics++'][ff_dict[label]]['test'][comp_name][video_name] = {
                                        'label': ff_dict[label],
                                        'frames': frame_paths,
                                        'masks': mask_frames_paths
                                    }
